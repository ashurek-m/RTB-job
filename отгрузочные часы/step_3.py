...
def shape_r62(shape_list, name_file):
    for i in range(len(shape_list)):
        if shape_list[i] == 62:
            ...
            columns_drop67 = ['price', 'расчёт', 'после анализа технологического отдела', 'расчёт.1',
                              'после анализа технологического отдела.1']
...
def shape_r61(shape_list, name_file):
    for i in range(len(shape_list)):
        if shape_list[i] == 61:
            name_file1 = name_file[i].replace('csv', 'xlsx')
            df_61 = pd.read_excel(name_file1)
